          ...
          <b-row>
            <b-col md="3">
              <b-form-group
                ...
                id="bfg-username"
                label="UserName"
                label-for="bfi-username"
                valid-feedback="ok"
              >
                <b-form-input
                  class="bg-dark customize border-0 mb-2 text-white"
                  id="bfi-username"
                  type="text"
            ...
            <b-col md="3">
              ...
              >
                <b-form-select
                  class="bg-dark customize text-white border-0"
                  :options="options"
                  v-model="form.cardId"
                >
            ...
            <b-col align-self="center" class="mt-3">
              <b-button variant="success" @click="save">Criar Usuário</b-button>
            </b-col>
          </b-row>
        </b-form>
        <hr class="bg-dark" />
...
export default {
  ...
  data: () => {
    return {
      form: { username: "", password: "", cardId: null },
      addUser: "",
      buttonLoad: false,
      currentPage: 1,
      fields: [
        ...
        { key: "cardUid", label: "Cartão", sortable: true },
  ...
  methods: {
    ...
    async loadCards() {
      try {
        ...
        const { status, data } = await response;
        if (status === 200) {
          this.options = data.map((card) => {
            return { value: card.id, text: card.uid };
          });
          this.buttonLoad = false;
        }
      } catch {
